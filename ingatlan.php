...
/* ===== Param ===== */
$id = (int)($_GET['id'] ?? 0);
if($id<=0){ http_response_code(404); exit('Hiányzó azonosító.'); }
...
WHERE i.id = :id
";
$st=$pdo->prepare($sql); $st->execute([':id'=>$id]); $rec=$st->fetch();
if(!$rec){ http_response_code(404); exit('A kért ingatlan nem található.'); }
  ...
  <div class="mt-3 grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <section class="lg:col-span-2">
      ...
      <div class="bg-white rounded-2xl shadow overflow-hidden">
        <img src="<?= e($hero) ?>" alt="" class="w-full h-80 object-cover">
        <?php if(count($kepek)>1): ?>
          <div class="p-3 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
            <?php foreach($kepek as $k): ?>
              ...
              <img src="<?= e($thumb) ?>" alt="" class="w-full h-20 object-cover rounded cursor-pointer"
    ...
    <section class="lg:col-span-1">
      <div class="bg-white rounded-2xl shadow p-5">
        ...
        <dl class="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
          <dt class="text-gray-500">Típus</dt><dd><?= e($rec['tipus'] ?? '—') ?></dd>
          <dt class="text-gray-500">Alapterület</dt><dd><?= e($rec['alapterulet'] ? $rec['alapterulet'].' m²' : '—') ?></dd>
          <dt class="text-gray-500">Szobaszám</dt><dd><?= e($rec['szobaszam'] ?? '—') ?><?= $rec['felszoba']?' + '.(int)$rec['felszoba'].' fél':'' ?></dd>
          <dt class="text-gray-500">Emelet</dt><dd><?= e($rec['emelet'] ?? '—') ?></dd>
          <dt class="text-gray-500">Falazat</dt><dd><?= e($rec['falazat'] ?? '—') ?></dd>
          <dt class="text-gray-500">Állapot</dt><dd><?= e($rec['allapot'] ?? '—') ?></dd>
          <dt class="text-gray-500">Státusz</dt><dd><?= e($rec['statusz'] ?? '—') ?></dd>
        </dl>
